#!/usr/bin/env python3
"""
Enhanced ESPN NFL Data Import and Team Update Script
...
3. Adds new players from ESPN to nfl_players table
"""
...
def clear_espn_players_table(cursor):
    ...
    print(f"ESPN players before clear: {count_before}")

    cursor.execute("DELETE FROM espn_players")
    rows_deleted = cursor.rowcount
    print(f"Cleared {rows_deleted} existing ESPN players")

    cursor.execute("SELECT COUNT(*) FROM espn_players")
    count_after = cursor.fetchone()[0]
    print(f"ESPN players after clear: {count_after}")
            ...
            for player in items:
                ...
                try:
                    ...
                    if player_data['espn_id'] and player_data['display_name'] and player_data['position']:
                        ...
                        if 'Bigsby' in player_data['display_name']:
                            print(f"  >>> Found Tank Bigsby: {player_data['display_name']} on {team_abbreviation} (ESPN ID: {player_data['espn_id']})")
...
def add_new_players_from_espn(cursor):
    """Add new players from ESPN to nfl_players table"""
    print("Adding new players from ESPN to nfl_players table...")
    ...
    print(f"Found {new_count} new players to add to nfl_players table")

    # Insert new players into nfl_players table
    insert_new_players_sql = """
    ...
    try:
        cursor.execute(insert_new_players_sql)
        added_rows = cursor.rowcount
        print(f"Added {added_rows} new players to nfl_players table")
        return added_rows
    except mysql.connector.Error as e:
        print(f"Error adding new players: {e}")
        return 0
...
def get_update_summary(cursor):
    ...
    cursor.execute("""
        SELECT
            ...
            COUNT(CASE WHEN np.espn_id IS NOT NULL AND np.espn_id != '' THEN 1 END) as with_espn_id
        FROM nfl_players np
        WHERE np.position IN ('QB', 'RB', 'RC', 'PK', 'DU')
    ...
    nfl_summary = cursor.fetchall()
    print(f"\nNFL players summary:")
    for pos, total, fa, team, with_espn in nfl_summary:
        print(f"  {pos}: {total} total ({team} on teams, {fa} free agents, {with_espn} with ESPN ID)")

def main():
    """Main function to fetch and import ESPN NFL players, then update team assignments"""
    print("Enhanced ESPN NFL Data Import & Team Update Script")
    print("=" * 60)
    ...
    try:
        ...
        # Step 5: Add new players from ESPN to nfl_players table
        print(f"\n" + "="*50)
        print("ADDING NEW PLAYERS")
        print("="*50)

        new_players_added = add_new_players_from_espn(cursor)
        connection.commit()
        ...
        print(f"- Updated {affected_rows} existing player team assignments")
        print(f"- Added {new_players_added} new players to nfl_players table")
